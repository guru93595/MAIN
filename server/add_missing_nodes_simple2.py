...
async def add_missing_nodes_simple():
    """Add POLLLLLL and LOOPPPPP nodes with ThingSpeak configuration"""
    
    async with engine.begin() as conn:
        # Add POLLLLLL (MAIN TANK) - using simpler format
        ...
        # Add LOOPPPPP (KRB TANK) - using simpler format
        await conn.execute(text("""
            INSERT INTO nodes (
                id, node_key, device_label, device_type, analytics_type, status, 
                lat, long, location_name, capacity,
...
if __name__ == "__main__":
    asyncio.run(add_missing_nodes_simple())
